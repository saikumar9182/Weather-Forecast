...
function HourlyForeCast({ dataInput,dataEnteredFlag,setDataEnteredFlag }) {
  ...
  if(dataEnteredFlag==true){
    ...
    const getLatitudeAndLongitude= async()=>{
       ...
       const res= await axios.get(`https://api.opencagedata.com/geocode/v1/json?q=${dataInput}&key=c9dd1926572844bfbac5c27f69c5f3ce`)
        ...
        const fetchForeCastWeather= async()=>{
          const res= await axios.get(`https://api.openweathermap.org/data/2.5/onecall?lat=${latitude}&lon=${longitude}&exclude=current,minutely,daily,alerts&appid=624bc7ea92b246f0df3f1b95d9df47f7`);
  ...
  return <div className="flexc" className="container">
      ...
      {eachHourContents.map((hour)=>(
          <div className="flexa" style={{border:"1px solid black"}}>
              ...
              <div>
                  <h3><i class="fas fa-wind" aria-hidden="true"></i>{hour.wind_speed}m/s</h3>
                  <h3><i class="fas fa-tint" aria-hidden="true"></i>{hour.humidity}%</h3>
                  <h2>{hour.dt}</h2>

              </div>

              <div>
              <h2 style={{fontSize:"50px",marginTop:"0"}}>{Math.ceil(hour.feels_like-273.15)}<span>&#8451;</span></h2>
